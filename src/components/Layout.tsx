...
const Layout = ({ children, isFooter = true, onFilterOpen, isHeader = true }: LayoutProps) => {
  ...
  const handleMenuItemClick = (path: string, text: string) => {
    ...
    if (text === 'Logout') {
      ...
      // Don't close mobile menu for settings
      setMobileOpen(true);
    } else {
      router.push(path);
    }
  ...
  return (
    <>
      {!isMobile && !isHomePage && pathname !== '/subscribe' && (
        <Sidebar
      ...
      <Box sx={{
        ...
        p: 0,
        m: 0,
        marginLeft: { md: isHomePage || pathname === '/subscribe' ? 0 : (isCollapsed ? '80px' : '250px') },
        width: { md: isHomePage || pathname === '/subscribe' ? '100%' : `calc(100% - ${isCollapsed ? '80px' : '250px'})` },
