...
import { formatStandardDateTime, formatStandardDate, formatStandardTime } from '@/utils/dateFormat';
...
const ActionHubDetail: React.FC<ActionHubDetailProps> = ({ alertId }) => {
  ...
  const getTimeAgo = (timestamp: string) => {
    ...
    if (diffInSeconds < 60) {
      ...
      // Format as DD MMM
      return formatStandardDate(alertTime);
    }
  };
    ...
    <Container maxWidth="xl" sx={{ padding: "0px" }}>
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Typography variant="h6" fontWeight="600" sx={{ mt: 1, mb: 1, fontSize: '16px', fontFamily: 'Poppins' }}>
              {alertData.title || 'Road Closures in 48h: Fringe Festival Protest'}
            </Typography>

            <Typography variant="body1" sx={{ mb: 2, fontSize: '14px', fontFamily: 'Inter', fontWeight: 500 }}>
          ...
          >
            {/* Start */}
            <Typography variant="body1" color="#757575" sx={{ fontSize: '14px', fontFamily: 'Poppins', fontWeight: 500 }}>Start</Typography>
            <Typography variant="body1" sx={{ fontSize: '14px', color: '#757575', fontFamily: 'Poppins', fontWeight: 500 }}>
              {alertData.expectedStart
                ? formatStandardDateTime(alertData.expectedStart)
                : '06 May 9:00AM'}
            </Typography>
            ...
            <Typography variant="body1" sx={{ fontSize: '14px', color: '#757575', fontFamily: 'Poppins', fontWeight: 500 }}>
              {alertData.expectedEnd
                ? formatStandardDateTime(alertData.expectedEnd)
                : '06 May 9:00AM'}
            </Typography>
          </Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: "column", md: "row" } }}>
        ...
        <Box sx={{
          ...
          {/* Activity Log */}
          <Typography variant="body1" fontWeight="600" sx={{ mb: 1, fontFamily: 'Poppins', fontSize: '16px' }}>
            Activity Log
          </Typography>
              ...
              <Box sx={{ height: 300, mb: 2, overflowY: "auto" }}>
                {actionLogs.map((log, index) => (
                  <Typography key={index} variant="body2" sx={{ py: 1, color: 'text.secondary' }}>
                    {log.formattedDate || (log.timestamp && formatStandardDate(log.timestamp))} {log.formattedTime || (log.timestamp && formatStandardTime(log.timestamp))} – {log.displayName || 'You'} {log.actionDetails || 'Unfollowed this alert by admin'}
                  </Typography>
                ))}
              </Box>
      ...
      >
        <DialogContent sx={{ textAlign: 'center', py: 3 }}>
          ...
          <DialogTitle sx={{ textAlign: 'center', fontWeight: '600', fontSize: '20px', fontFamily: 'Poppins' }}>
            Send this alert now?
          </DialogTitle>
          <DialogContentText sx={{ fontSize: '14px', fontFamily: 'Poppins' }}>
      ...
      >
        <DialogContent sx={{ textAlign: 'center', p: 1 }}>
          ...
          <DialogTitle sx={{ textAlign: 'center', fontWeight: '600', fontSize: '20px', fontFamily: 'Poppins', p: 1 }}>
